---
name: bash-column
description: "Program the column command: Format input into aligned columns."
version: 1.0.0
...
---
    ...
    Use `column` when your task matches what it does best (see Overview). In a
...
```
    ...
    `column` is a building block in a shell pipeline. Compose it with `|`, guard on
    ...
    if column ... ; then
        ...
        echo "column failed with exit $?" >&2
        exit 1
    fi
    ```
    ...
    ## Full reference (`column --help` on this machine)
