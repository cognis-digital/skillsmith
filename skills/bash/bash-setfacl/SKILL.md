---
name: bash-setfacl
description: "Program the setfacl command: Set POSIX access-control lists on files."
version: 1.0.0
...
---
    ...
    Set POSIX access-control lists on files.

    ## When to use

    Use `setfacl` when your task matches what it does best (see Overview). In a
...
```
    ...
    `setfacl` is a building block in a shell pipeline. Compose it with `|`, guard on
    ...
    if setfacl ... ; then
        ...
        echo "setfacl failed with exit $?" >&2
        exit 1
    fi
    ```
    ...
    ## Full reference (`setfacl --help` on this machine)
...
For each file given as parameter, setfacl will either replace its complete ACL
