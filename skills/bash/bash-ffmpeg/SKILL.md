---
name: bash-ffmpeg
description: "Program the ffmpeg command: Convert, trim, and transform audio and video."
version: 1.0.0
tags: [bash, cli, command-line, media]
---
    ...
    Convert, trim, and transform audio and video.

    ## When to use

    Use `ffmpeg` when your task matches what it does best (see Overview). In a
...
```
```bash
ffmpeg -i v.mp4 -vf scale=640:-1 s.mp4
```

    ## Structuring it in a program

    `ffmpeg` is a building block in a shell pipeline. Compose it with `|`, guard on
    ...
    if ffmpeg ... ; then
        ...
        echo "ffmpeg failed with exit $?" >&2
        exit 1
    fi
    ```
    ...
    ## Full reference (`ffmpeg --help` on this machine)
