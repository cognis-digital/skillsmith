---
name: bash-strace
description: "Program the strace command: Trace system calls and signals a process makes — debugging's X-ray."
version: 1.0.0
tags: [bash, cli, command-line, debug]
---
    ...
    Trace system calls and signals a process makes — debugging's X-ray.

    ## When to use

    Use `strace` when your task matches what it does best (see Overview). In a
...
```
    ...
    `strace` is a building block in a shell pipeline. Compose it with `|`, guard on
    ...
    if strace ... ; then
        ...
        echo "strace failed with exit $?" >&2
        exit 1
    fi
    ```
    ...
    ## Full reference (`strace --help` on this machine)
