---
name: git-rebase
description: "Program git rebase: Reapply commits on top of another base tip (rewrites history)."
version: 1.0.0
...
---
    ...
    Reapply commits on top of another base tip (rewrites history).

    ## When to use

    `git rebase` is part of an everyday version-control workflow. Know exactly what
    it changes before you run it — some git operations rewrite history.

    ## Worked example

    ```bash
    git rebase --help     # read the options first
    git rebase ...        # then run it deliberately
    ```
    ...
    ```bash
    git rebase ... || { echo "git rebase failed" >&2; exit 1; }
    git status --short
    ```

    ## Full reference (`git rebase --help`)
