---
name: git-fetch
description: "Program git fetch: Download objects and refs from another repository."
version: 1.0.0
...
---
    ...
    `git fetch` is part of an everyday version-control workflow. Know exactly what
    it changes before you run it — some git operations rewrite history.

    ## Worked example

    ```bash
    git fetch --help     # read the options first
    git fetch ...        # then run it deliberately
    ```
    ...
    ```bash
    git fetch ... || { echo "git fetch failed" >&2; exit 1; }
    git status --short
    ```

    ## Full reference (`git fetch --help`)
