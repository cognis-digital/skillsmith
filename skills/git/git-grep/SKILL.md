---
name: git-grep
description: "Program git grep: Search tracked files for a pattern."
version: 1.0.0
...
---
    ...
    `git grep` is part of an everyday version-control workflow. Know exactly what
    it changes before you run it — some git operations rewrite history.

    ## Worked example

    ```bash
    git grep --help     # read the options first
    git grep ...        # then run it deliberately
    ```
    ...
    ```bash
    git grep ... || { echo "git grep failed" >&2; exit 1; }
    git status --short
    ```

    ## Full reference (`git grep --help`)
