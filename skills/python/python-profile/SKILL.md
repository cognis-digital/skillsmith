---
name: python-profile
description: "Program with Python's profile module: Class for profiling Python code."
version: 1.0.0
tags: [profile, programming, python, stdlib]
---
...
Reach for `profile` when your task calls for Class for profiling Python code. It ships with Python — no dependency to install, available on every interpreter.
...
- `profile.runctx(statement, globals, locals, filename=None, sort=-1)`
...
def do_work(...):
    """Use profile to accomplish one well-defined task."""
    result = profile.main(...)
    return result
```
...
- Keep `profile` calls behind a small function so the rest of your code does not
  depend on its details.

## Full reference (introspected from this machine)

```
Python Library Documentation: module profile
...
Other standard-library modules pair well with `profile`; explore the `python` domain of this catalog.
